...
export const analyzeScreenshotWithOpenAI = async ({ dataUrl, filename = "screenshot.png" }) => {
  ...
  const model = (import.meta.env.VITE_OPENAI_VISION_MODEL || import.meta.env.VITE_OPENAI_MODEL || "gpt-4o-mini").trim();
  const requestUrl = buildOpenAIUrl(import.meta.env.VITE_OPENAI_API_URL || "https://api.openai.com/v1/chat/completions");

  if (!apiKey) {
    throw new Error("未配置 VITE_OPENAI_API_KEY，无法解析截图。");
  }
...
2. details 最多 4 条，只保留对销售/关系维护/保单方案有价值的信息；
3. tags 最多 4 个，必须是自然语言短语；
4. suggestedActions 最多 3 条，必须具体可执行；
5. 如果截图信息不足，明确写“截图信息有限”，不要编造。`
  ...
  if (!resp.ok) {
    ...
    throw new Error(`截图解析失败 (${resp.status}): ${text.slice(0, 500)}`);
  }

  const body = await resp.json();
  if (body?.error) {
    throw new Error(body.error.message || "截图解析失败");
  }

  const rawText = extractTextFromModelResponse(body);
  const parsed = parseJson(rawText);
  if (!parsed) {
    throw new Error("截图解析结果不是有效 JSON。");
  }
